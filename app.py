...
labels = ['On-time', 'Can be completed on-time using float', 'Delayed']
...
top_n_tasks = sorted(dependency_map.items(), key=lambda x: len(x[1]), reverse=True)[:16]
...
cascade_series.plot(kind='bar', color='#ff930f', ax=ax)
...
st.metric(
    ...
    value=f"{on_track_percent:.2f}%",
    help="Percentage of in-progress tasks projected estimated to finish on or before time"
)
...
on_time_percentage = start_stats[True]

st.metric(
    label="Before or On-Time Start Rate (Completed and In-Progress tasks)",
    value=f"{on_time_percentage:.2f}%",
    help="Percentage of completed and in-progress tasks that started on or before their planned start date"
)
